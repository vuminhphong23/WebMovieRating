...
export const AppRatingDistribution = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        const response = await fetch(
          "https://sacred-beetle-closely.ngrok-free.app/app-rating-distribution/",
          {
            headers: {
        ...
        setData(result);
        setTotal(result.reduce((acc, item) => acc + item.count, 0)); // Tính tổng số reviews
        ...
        plugins: [
          {
            ...
            ctx.fillStyle = '#45b098';
              ctx.fillText(`${total}`, centerX - 20, centerY - 10); // Tổng số
              ctx.font = 'normal 15px Arial';
              ctx.fillText('Total reviews', centerX - 20, centerY + 20); // Nhãn
              ctx.restore();
            },
          },
        ],
  ...
  return (
    <>
      {error && <div>{error}</div>}
      <div style={{ display:"flex",flexWrap:"wrap", justifyContent: "start", color:"#45b098" }}>
        <div>
          <input
            type="checkbox"
            id="select-all-app"
          ...
          />
          <label style={{ padding: 5, fontSize: 18 }} htmlFor="select-all-app">All</label>
        </div>
        {data.map((item) => (
          <div key={item.rating}>
            <input
              type="checkbox"
              id={`rating-app-${item.rating}`}
              checked={selectedRatings.includes(item.rating)}
              onChange={() => handleCheckboxChange(item.rating)}
            />
            <label style={{ padding: 5, fontSize: 18 }} htmlFor={`rating-app-${item.rating}`}>{`${item.rating} star`}</label>
          </div>
        ))}
      </div>
      <canvas ref={chartRef} id="myChart-app"></canvas>
    </>
  );
};
